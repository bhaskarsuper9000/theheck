...
from theheck.utils import default_settings, \
...
class TestGetCloseMatches(object):
    @patch('theheck.utils.difflib_get_close_matches')
...
def get_aliases(mocker):
    mocker.patch('theheck.shells.shell.get_aliases',
                 return_value=['vim', 'apt-get', 'fsck', 'heck'])
...
def test_get_all_executables():
    ...
    assert 'heck' not in all_callables
...
class TestCache(object):
    ...
    def shelve(self, mocker):
        ...
        mocker.patch('theheck.utils.shelve.open', new_callable=lambda: _Shelve)
        return value

    @pytest.fixture(autouse=True)
    def enable_cache(self, monkeypatch, shelve):
        monkeypatch.setattr('theheck.utils.cache.disabled', False)
        _cache._init_db()

    @pytest.fixture(autouse=True)
    def mtime(self, mocker):
        mocker.patch('theheck.utils.os.path.getmtime', return_value=0)
    ...
    def key(self, monkeypatch):
        monkeypatch.setattr('theheck.utils.Cache._get_key',
                            lambda *_: 'key')
        return 'key'
...
class TestGetValidHistoryWithoutCurrent(object):
    ...
    def history(self, mocker):
        return mocker.patch('theheck.shells.shell.get_history',
                            return_value=['le cat', 'heck', 'ls cat',
                                          'diff x', 'nocommand x', u'café ô'])

    @pytest.fixture(autouse=True)
    def alias(self, mocker):
        return mocker.patch('theheck.utils.get_alias',
                            return_value='heck')
    ...
    def bins(self, mocker):
        ...
        return mocker.patch('theheck.utils.Path', return_value=path_mock)

    @pytest.mark.parametrize('script, result', [
        ('le cat', ['ls cat', 'diff x', u'café ô']),
        ('diff x', ['ls cat', u'café ô']),
        ('heck', ['ls cat', 'diff x', u'café ô']),
