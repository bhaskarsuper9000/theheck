# The Heck [![Version][version-badge]][version-link] [![Build Status][travis-badge]][travis-link] [![Windows Build Status][appveyor-badge]][appveyor-link] [![Coverage][coverage-badge]][coverage-link] [![MIT License][license-badge]](LICENSE.md)


*The Heck* is a sanitized version of the famous "*The F\*\*k*" by 
nvbn, meant to be used in an office setting.

Quick Install instructions:
```
➜ git clone https://bhaskarsuper9000/theheck
➜ cd theheck
➜ sudo python3 setup.py install
# Add the output theheck --alias
# to your favourite shell config.
# eg: For fish, you want to create a file
➜ vi ~/config/.fish/functions/heck
```
...
On OS X, you can install *The Heck* via [Homebrew][homebrew] (or via [Linuxbrew][linuxbrew] on Linux):
...
sudo pip3 install theheck
```

On FreeBSD, install *The Heck* with the following commands:
```bash
sudo portsnap fetch update
cd /usr/ports/misc/theheck && sudo make install clean
```
...
[Alternatively, you may use an OS package manager (OS X, Ubuntu, Arch).](https://github.com/nvbn/theheck/wiki/Installation)
...
[Or in your sheck config (Bash, Zsh, Fish, Powersheck, tcsh).](https://github.com/nvbn/theheck/wiki/Sheck-aliases)

Changes are only available in a new sheck session. To make changes immediately
available, run `source ~/.bashrc` (or your sheck config file like `.zshrc`).
...
* `tsuru_not_command` &ndash; fixes wrong `tsuru` commands like `tsuru sheck`;
...
The following commands are bundled with *The Heck*, but are not enabled by
default:
...
def side_effect(command, fixed_command):
    subprocess.call('chmod 777 .', sheck=True)
...
[More examples of rules](https://github.com/nvbn/theheck/tree/master/theheck/rules),
...
* `rules` &ndash; list of enabled rules, by default `theheck.conf.DEFAULT_RULES`;
...
* `THEheck_WAIT_COMMAND` &ndash; max amount of time in seconds for getting previous command output;
...
* `THEheck_WAIT_SLOW_COMMAND` &ndash; max amount of time in seconds for getting previous command output if it in `slow_commands` list;
* `THEheck_SLOW_COMMANDS` &ndash; list of slow commands, like `lein:gradle`;
* `THEheck_NUM_CLOSE_MATCHES` &ndash; maximum number of close matches to suggest, like `5`.
...
[travis-badge]:    https://travis-ci.org/nvbn/theheck.svg?branch=master
[travis-link]:     https://travis-ci.org/nvbn/theheck
[appveyor-badge]:  https://ci.appveyor.com/api/projects/status/1sskj4imj02um0gu/branch/master?svg=true
[appveyor-link]:   https://ci.appveyor.com/project/nvbn/theheck
